...
public class ConfirmationLetterGenerator {
	...
	public OurOwnByteArrayOutputStream letter(RequestContext context, FileUploadCommand fileUploadCommand,
			...
			List<AmountAndRecordsPerBank> bankMap, List<FaultRecord> faultyRecords, FileExtension extension,
	...
	// List<BigDecimal> retrievedAmountUSD
		...
		letter.setHashTotalDebit(hashBatchRecordsBalance.getHashTotalCredit().toString());
		...
		if (fileUploadCommand.getFee().equalsIgnoreCase(Constants.YES.toString())) {
		...
		retrievedAmounts = calculateRetrieveAmounts(records, faultyRecords, client, extension,
				faultyAccountNumberRecordList, sansDuplicateFaultRecordsList);
		...
		System.out.println("TRACING AMOUNT["+letter.getRetrievedAmountFL()+"]")+letter.setRetrievedAmountFLDBF(retrievedAmounts.get("FLDBF"));
		...
		context.getConversationScope().put("dsbByteArrayOutputStream", arrayOutputStream);
	...
	private Map<String, BigDecimal> calculateAmountsFaultyAccountNumber(List<TempRecord> faultyAccountNumberRecordList,
			Client client) {
		...
		for (TempRecord faultyAccountNumberRecord : faultyAccountNumberRecordList) {
			...
			if (faultyAccountNumberRecord.getCurrencycode().equals(Constants.FL_CURRENCY_CODE)
				...
				if (faultyAccountNumberRecord.getSign().equalsIgnoreCase(Constants.DEBIT.toString())) {
	...
	private Map<String, BigDecimal> calculateRetrieveAmounts(List<Record> records, List<FaultRecord> faultyRecords,
			Client client, FileExtension extension, List<TempRecord> faultyAccountNumberRecordList,
			List<TempRecord> sansDuplicateFaultRecordsList) {
		...
		if (client.getCounterTransfer().equalsIgnoreCase(Constants.TRUE.toString())) {
			for (Record record : records) {
				if (record.getFeeRecord() != 1) {
					if ((record.getCurrency().getCode().equals(Constants.FL_CURRENCY_CODE)
							...
							&& record.getSign().equalsIgnoreCase(Constants.DEBIT.toString())) {
						recordAmountFL = record.getAmount().add(recordAmountFL);
						// system.out.println("recordAmountFL: ["+
						// recordAmountFL + "]");
					...
					if (record.getCurrency().getCode().equals(Constants.EUR_CURRENCY_CODE)
						...
						// system.out.println("recordAmountEUR: ["+
						// recordAmountEUR + "]");
					...
					if (record.getCurrency().getCode().equals(Constants.USD_CURRENCY_CODE)
						...
						// system.out.println("recordAmountUSD: ["+
						// recordAmountUSD + "]");
					}
				}
				retrievedAmounts.put(Constants.CURRENCY_EURO.toString(), recordAmountEUR);
				retrievedAmounts.put(Constants.CURRENCY_FL.toString(), recordAmountUSD);
				retrievedAmounts.put(Constants.CURRENCY_FL.toString(), recordAmountFL);
			}
		}
		// Not Balanced
		else {
			...
			for (Record record : records) {
				logger.debug("COUNTERTRANSFER [" + record.getIsCounterTransferRecord() + "] FEERECORD ["
						+ record.getFeeRecord() + "]");
				if (record.getIsCounterTransferRecord().compareTo(new Integer(0)) == 0
						&& record.getFeeRecord().compareTo(new Integer(0)) == 0) {
					if ((record.getCurrency().getCode().equals(Constants.FL_CURRENCY_CODE)
							|| record.getCurrency().getCode().equals(Constants.FL_CURRENCY_CODE_FOR_WEIRD_BANK))) {
						// System.out.println("record to string:
						// ["+record.toString()+"]");
						if (record.getSign().equalsIgnoreCase(Constants.DEBIT.toString())) {
							// System.out.println("record.getamount DEBIT = ["+
							// record.getAmount() + "]");
							// system.out.println("recordAmountDebitFL 1 = "+
							// recordAmountDebitFL);
							recordAmountDebitFL = record.getAmount().add(recordAmountDebitFL);
							// System.out.println("recordAmountDebitFL:
							// ["+recordAmountDebitFL+"]");
						}
						if (record.getSign().equalsIgnoreCase(Constants.CREDIT.toString())) {
							// System.out.println("record.getamount CREDIT =
							// ["+record.getAmount()+"]");
							// system.out.println("recordAmountCreditFL 1 =
							// ["+recordAmountCreditFL+"]");

							recordAmountCreditFL = record.getAmount().add(recordAmountCreditFL);
							// System.out.println("recordAmountCreditFL:
							// ["+recordAmountCreditFL+"]");
						}
			...
			for (TempRecord sansDupRec : sansDuplicateFaultRecordsList) {
				...
				if (currencyCode == null) {
					...
					if (currencyCode.equals(Constants.FL_CURRENCY_CODE)
						...
						if (sansDupRec.getSign().equalsIgnoreCase(Constants.DEBIT.toString())) {
			...
			if (retrievedAccountNumberAmounts.get("FaultyAccDebitFL") != null && amountSansDebitFL != null) {
				// logger.info("retrievedAccountNumberAmounts.get(FaultyAccDebitFL)
			...
			if (retrievedAccountNumberAmounts.get("FaultyAccDebitUSD") != null && amountSansDebitUSD != null) {
				// logger.info("retrievedAccountNumberAmounts.get(FaultyAccDebitUSD)
			...
			if (retrievedAccountNumberAmounts.get("FaultyAccDebitEUR") != null && amountSansDebitEUR != null) {
				// logger.info("retrievedAccountNumberAmounts.get(FaultyAccDebitEUR)
	...
	private List<AmountAndRecordsPerBank> getAmountAndRecords(Record record, List<AmountAndRecordsPerBank> list,
			String transactionType) {
		Map<String, String> map = new HashMap<String, String>();
		if (record.getFeeRecord().compareTo(0) == 0 && !map.containsKey(record.getBeneficiaryName())) {

			if (transactionType.equalsIgnoreCase(Constants.CREDITING.toString())) {

				if (record.getBeneficiaryName() != null
						&& !record.getBeneficiaryName().equalsIgnoreCase(Constants.RBTT_BANK_ALTERNATE.toString())) {
					Boolean newList = true;
					if (list.size() == 0 && record.getSign().equalsIgnoreCase(type)) {
						// logger.info("bank gegevens: "+record.getSign()+" :
						// "+record.getBank().getName()+" :
						// "+record.getBeneficiaryName());
					...
					if (newList && record.getSign().equalsIgnoreCase(type)) {
						...
						for (AmountAndRecordsPerBank object : list) {
							if (object.getBankName().equalsIgnoreCase(record.getBank().getName()) && object
									.getCurrencyType().equalsIgnoreCase(record.getCurrency().getCurrencyType())) {
			...
			// del begin
			if (transactionType.equalsIgnoreCase(Constants.DEBITING.toString())) {
